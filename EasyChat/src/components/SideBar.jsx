...
export default function SideBar() {
  ...
  const {users, selectedUser, getUsers, sendMessages,
        setSelectedUser, unseenMessages, setUnseenMessages} = useContext(ChatContext);
  const [input, setInput] = useState(false);
  const filteredUsers = input ? users.filter((user) => user.fullName.tolowerCase().includes(
    input.toLowerCase()
  )) : users;
        ...
        <div className="flex justify-between items-center">
            ...
            <div className="relative py-2 group">
                ...
                border border-gray-600 text-gray-100 hidden group-hover:block">
                    <Link to='/profile' className="cursor-pointer text-sm">Edit Profile</Link>
